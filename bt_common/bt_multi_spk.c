...
void BTMSPK_EventHandler( BT_MSPK_EVENTS event, uint8_t* paras, uint16_t size )
{
    ...
    switch(event)
    {
        case BT_EVENT_NSPK_STATUS:
			...
			User_Log("BT_eCSBStatus.nspk_link = %d\n\n",BT_eCSBStatus.nspk_link);
			User_Log("BT_eCSBStatus.snpk_event = %d\n\n",BT_eCSBStatus.snpk_event);
...
//#endif                            
							BTVOL_SyncVolFromMasterToSlaveByVendorCmd();	////2019-5-27, set slave volume when grouped
                        }
...
//#endif
            ...
            {
				User_Log("BT_eCSBStatus.nspk_status = %d\n\n",BT_eCSBStatus.nspk_status);
