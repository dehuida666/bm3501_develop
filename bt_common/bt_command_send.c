...
void BT_DisconnectBTClassic(void)
{
    uint8_t command[6];
    command[0] = 0xAA;                      //header byte 0
    command[1] = 0x00;                      //header byte 1
...
/*------------------------------------------------------------*/
void BT_PlayTone(uint8_t tone_index)
{
    uint8_t command[7];
    command[0] = 0xAA;      //header byte 0
    command[1] = 0x00;      //header byte 1
...
/*------------------------------------------------------------*/
#if 0     
void BT_UpdateBatteryLevel(uint8_t level)
{
    uint8_t command[6];
    command[0] = 0xAA;      //header byte 0
    command[1] = 0x00;      //header byte 1
    command[2] = 2;         //length
    command[3] = BATTERY_LEVEL_INDICATION;      //command ID
    command[4] = level;  
    command[5] = calculateChecksum(&command[2], &command[4]);
    copyDataToBufferAndRegisterCommand(&command[0], 6);    
}
...
void BT_UpdateBatteryLevel(uint8_t level)
{
	uint8_t level_tmp;
    uint8_t command[24] = {
		0xAA, 0x00, 0x14, 0x0A, 0x00, 0x2B, 0x49, 0x50, 0x48, 0x4F, 0x4E, 0x45, 
		0x41, 0x43, 0x43, 0x45, 0x56, 0x3D, 0x31, 0x2C, 0x31, 0x2C, 0x30, 0x6B
		};     

	/*
	level : 0~100   10 steps
	The data that will be sent ranges form 0x30 to 0x39. 
	0x30: 10%
	0x31: 20%
	0x32: 30%
	0x33: 40%
	...
	*/
    ...
    command[22] = level_tmp;  
    command[23] = calculateChecksum(&command[2], &command[22]);
    copyDataToBufferAndRegisterCommand(&command[0], 24);    
}
#endif
/*------------------------------------------------------------*/
void BT_SetVoicePromptByRawData(void)
{
    uint8_t command[7];
    command[0] = 0xAA;                      //header byte 0
    command[1] = 0x00;                      //header byte 1
    command[2] = 0x03;                      //length
    command[3] = VOICE_PROMPT_CMD;      //command ID
    command[4] = 0x00;                      
    command[5] = 0x01;                      
    command[6] = calculateChecksum(&command[2], &command[5]);
    copyDataToBufferAndRegisterCommand(&command[0], 7);
...
void BT_PlayVoicePromptByRawData(uint8_t* data, uint16_t size)

{
    uint16_t i, size_this, size_cmd;
    uint8_t command[500];
    uint8_t fragment = 0;
    if(size > 480)
    {
        while(size)
        {
            ...
            size_cmd = size_this + 3;
            command[0] = 0xAA;
            command[1] = (unsigned char)(size_cmd>>8);
            command[2] = (unsigned char)(size_cmd&0x00ff);
            command[3] = VOICE_PROMPT_CMD;      //command ID
            command[4] = 0x01;
            command[5] = fragment;
            ...
            command[6+size_this] = calculateChecksum(&command[1], &command[5+size_this]);
            copyDataToBufferAndRegisterCommand(&command[0], 7+size_this);
        }
    }
    else
    {
        ...
        command[6+size] = calculateChecksum(&command[1], &command[5+size]);
        copyDataToBufferAndRegisterCommand(&command[0], 7+size);
    }
}
        ...
        case BT_CMD_SEND_ACK_ERROR:
            if (!BT_CommandSendTimer) {
                if (BT_SendingCmd.SendingCmdNum) {
                    ...
                    BM6X_MFB_SetLow();
					User_Log("BT_CMD_SEND_ACK_ERROR,BM6X_MFB_SetLow\n");
