...
void BT_CommandDecode( void )
{
	...
	User_Log("BT_CmdBuffer[0] = %02x\n", BT_CmdBuffer[0]);
	User_Log("BT_CmdBuffer[1] = %02x\n", BT_CmdBuffer[1]);
	#endif
    ...
    switch(BT_CmdBuffer[0])
    {
		...
		case RINGTONE_FINISH_INDICATION://add by zx
			BTAPP_EventHandler(BT_EVENT_RINGTONE_INDICATION, &BT_CmdBuffer[1], BT_CmdDecodeCmdLength-1 );
			break;
